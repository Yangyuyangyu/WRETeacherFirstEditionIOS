...
{
    UITableView *chooseTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.tapView.frame.size.width, self.tapView.frame.size.height)];
    ...
    [_chooseTableView registerNib:[UINib nibWithNibName:@"ImagePickerTableViewCell" bundle:nil] forCellReuseIdentifier:@"cell"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ImagePickerTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    if (indexPath.row == 0) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ShareS.isZero = YES;
        [[NSNotificationCenter defaultCenter] postNotificationName:@"takingPicturesInfoList" object:nil userInfo:nil];
        [self disappear];
    }
    //从手机相册选择
    else if (indexPath.row == 1)
